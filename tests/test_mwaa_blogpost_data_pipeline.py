...
class TestMWAABlogpostDataPipeline(unittest.TestCase):
    ...
    def test_dag_import_without_variables_mock(self):
        """Test DAG import without variable mocking to reproduce MWAA deployment failure"""
        # Stop the variable mock to test real import behavior
        self.variable_patcher.stop()

        try:
            # Clear cached modules for fresh import
            import sys
            if 'scripts.mwaa_blogpost_data_pipeline' in sys.modules:
                del sys.modules['scripts.mwaa_blogpost_data_pipeline']

            from scripts.mwaa_blogpost_data_pipeline import dag
            self.assertIsNotNone(dag)
            print("✅ DAG import successful without mocks")
        except KeyError as e:
            if "glue_crawler_name" in str(e):
                print(f"🔴 Reproduced CloudWatch error: {e}")
                self.fail(f"DAG import failed due to missing variable: {e}")
            else:
                self.fail(f"Unexpected KeyError: {e}")
        except Exception as e:
            self.fail(f"Unexpected error importing DAG without mocks: {e}")
        finally:
            # Restart the mock for other tests
            self.variable_patcher.start()
...
class TestSyntaxAndImports(unittest.TestCase):
    ...
    def test_airflow_imports(self):
        ...
        try:
            ...
            from airflow.models import Variable, BaseOperator  # noqa: F401

            # Test AWS provider imports
            from airflow.providers.amazon.aws.sensors.s3 import S3KeySensor  # noqa: F401
            from airflow.providers.amazon.aws.hooks.s3 import S3Hook  # noqa: F401
            from airflow.providers.amazon.aws.operators.emr import EmrAddStepsOperator, EmrCreateJobFlowOperator  # noqa: F401
            from airflow.providers.amazon.aws.sensors.emr import EmrStepSensor  # noqa: F401
            from airflow.providers.amazon.aws.operators.athena import AthenaOperator  # noqa: F401
            from airflow.providers.amazon.aws.hooks.base_aws import AwsBaseHook  # noqa: F401
